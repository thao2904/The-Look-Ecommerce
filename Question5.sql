...
JOIN 
    bigquery-public-data.thelook_ecommerce.orders AS b 
    ON a.order_id = b.order_id
JOIN 
    bigquery-public-data.thelook_ecommerce.products AS c 
    ON a.id = c.id
WHERE 
    DATE(a.created_at) BETWEEN DATE_SUB(DATE('2022-04-15'), INTERVAL 3 MONTH) 
    AND DATE('2022-04-15')
GROUP BY  
    c.category, FORMAT_DATE('%Y-%m-%d', a.created_at)
ORDER BY 
    date;
